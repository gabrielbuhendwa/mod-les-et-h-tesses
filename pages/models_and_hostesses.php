<?php 
    require('../controllers/models_and_hostessesController.php');
...
<head>
    ...
    <title>Models & Hostesses</title>
</head>
<body>
<?php include '../includes/head.php'?>
    <section class="hero-section">
        <h2 class="gallery-title">Réservez un modèle ou une hôtesse</h2>
        <div class="gallery">
            <?php
            // Display models
            foreach ($models as $model) {
                echo '
                <div class="card" onclick="redirectToReservation(' . $model['id'] . ')">
                    <img src="../asset/images/models/' . htmlspecialchars($model['full_body_front_side']) . '" alt="' . htmlspecialchars($model['first_name'] . ' ' . $model['last_name']) . '">
                    <h3>' . strtoupper(htmlspecialchars($model['first_name'])) . ' ' . strtoupper(htmlspecialchars($model['last_name'])) . '</h3>
            ...
            foreach ($hostesses as $hostess) {
                echo '
                <div class="card" onclick="redirectToReservation(' . $hostess['id'] . ')">
                    <img src="../asset/images/models/' . htmlspecialchars($hostess['full_body_front_side']) . '" alt="' . htmlspecialchars($hostess['first_name'] . ' ' . $hostess['last_name']) . '">
                     <h3>' . strtoupper(htmlspecialchars($hostess['first_name'])) . ' ' . strtoupper(htmlspecialchars($hostess['last_name'])) . '</h3>
                    <div class="card-details">
