...
try {
    ...
    $query = $bdd->prepare("SELECT * FROM hostesses_requests WHERE id = :id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['validate'])) {
    try {
        ...
        $delete = $bdd->prepare("DELETE FROM hostesses_requests WHERE id = :id");
        ...
        $_SESSION['status'] = "La candidature a été validée avec succès. Le candidat(e) a été informé(e) et est désormais visible dans la galerie et accessible pour les réservations.";
                                echo '<script>
                                        setTimeout(function() {
                                            window.location.href = "../admin/hostesses_requests.php";
                                        }, 9000);
                                    </script>';
    } catch (Exception $e) {
        $bdd->rollBack();
